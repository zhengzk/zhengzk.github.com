base.clazz('base.component.TabNavi', function(_cfg){
	this.cfg=base.cfgDefault(_cfg,{
		...
		cb_select: null,
		default_index:0,
		class_name:'stc-component-tab-navi',
		render:null,
		self:false,
		flag:true,//是否触发默认事件
		loading:null
	...
	if(null != this.cfg.loading){
		...
		$(this.loading).html('&nbsp;');
		$(this.loading).addClass('stc-component-tab-navi-cell-active');
		$(this.loading).css(IMAGES.IMAGE.LOADING.SIZE16);
		$(this.loading).addClass('stc-component-tab-navi-loading'); 
	}
...
return{
	...
	insertTabNavi:function(_data){
		...
		for(var k in _data){
			...
			$(_col).addClass('stc-component-tab-navi-cell inline');
	...
	bindFun:function(_obj,data){
		...
		var active_class = 'stc-component-tab-navi-cell-active';
		var default_class = 'stc-component-tab-navi-cell';
